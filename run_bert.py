...
# Fix to allow loading numpy globals in checkpoints with weights_only=True (default in torch 2.6+)
try:
    import numpy as np
    from numpy._core.multiarray import _reconstruct
    safe_globals = [_reconstruct, np.ndarray, np.dtype]
    # Add specific numpy dtypes that might be in RNG states
    if hasattr(np, 'dtypes') and hasattr(np.dtypes, 'UInt32DType'):
        safe_globals.append(np.dtypes.UInt32DType)
...
import transformers
from models.patch_bert_flash import apply_flash_bert_patch
apply_flash_bert_patch()
...
class DataTrainingArguments:
    ...
    target_total_batch_size: int | None = field(
        default=None,
...
def main():
    ...
    if data_args.dataset_name is not None:
        if os.path.exists(os.path.join(data_args.dataset_name, "dataset_dict.json")):
    ...
    if tokenizer.mask_token is None:
        logger.info("Tokenizer does not have a mask token. Adding [MASK] token.")
        tokenizer.add_special_tokens({'mask_token': '[MASK]'})

    if tokenizer.pad_token is None:
        logger.info("Tokenizer does not have a pad token. Adding [PAD] token.")
