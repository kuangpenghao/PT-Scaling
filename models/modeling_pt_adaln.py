...
class PtEncoderIterator(nn.Module):
    ...
    def forward(
        ...
        if gamma is not None and beta is not None:
            # gamma, beta: (dim_z,) -> broadcast to (bsz, seq_len, dim_z)
            qz = qz * (1 + gamma) + beta
...
class PtModel(PtPreTrainedModel):
    ...
    def __init__(self, config: PtConfig):
        ...
        # AdaLN parameters: Gamma (scale) and Beta (shift) for each iteration
        self.ada_gamma = nn.Parameter(torch.zeros(config.num_iterations, config.dim_z))
        self.ada_beta = nn.Parameter(torch.zeros(config.num_iterations, config.dim_z))
    ...
    def forward(
        ...
        for idx in range(self.config.num_iterations):
            ...
            # Retrieve AdaLN params for this layer/iteration
            gamma_t = self.ada_gamma[idx]
            beta_t = self.ada_beta[idx]
